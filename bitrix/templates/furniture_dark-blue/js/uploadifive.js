...
(function (b) {
  var a = {
      ...
      return this.each(function () {
          ...
          j.bind('change', function () {
            ...
            if ((d.queue.count + l) > f.queueSizeLimit && f.queueSizeLimit !== 0) {
              if (b.inArray('onError', f.overrideEvents) < 0) {
                showError('Нельзя выбрать больше одного файла в сообщение');
              }
        ...
        d.addQueueItem = function (j) {
          if (j.size > f.fileSizeLimit && f.fileSizeLimit != 0) {
			...
			showError('Вложенные файлы должны быть не более 60МБ.<br>'+j.name+' не может быть вложен в сообщение');
			return false;
			}
        ...
        d.removeQueueItem = function (m, l, k) {
          ...
          var j = l ? 0 : 300;
          if (m.queueItem) {
            if (m.queueItem.find('.fileinfo').html() != 'Доставлен') {
              m.queueItem.find('.fileinfo').html('Отменен');
            }
            m.queueItem.find('.progress-bar').width(0);
            m.queueItem.delay(k).fadeOut(j, function () {
              b(this).remove();
			  if($('#usr-filename .msg_file').length == 0) {
				$('#usr-filename .att_text').remove();
			  }
          ...
          if (b.inArray('onError', f.overrideEvents) < 0) {
            switch (l) {
              ...
              case 'FILE_SIZE_LIMIT_EXCEEDED':
                errorMsg = 'Недопустимый размер файла';
                break;
              default:
        ...
        d.uploadComplete = function (l, j, k) {
          if (b.inArray('onUploadComplete', f.overrideEvents) < 0) {
            ...
            j.queueItem.find('.fileinfo').html('Доставлен');
    ...
    appendFormData: function (key, value) {
      return this.each(function () {
		b(this).data('uploadifive').settings.formData[key] = value;
      });
    }
 };
